(function collectionsPage() {
  ...
  function menuHTML(pack, own) {
    ...
    if (own) {
      ...
      <div class="cabinet-menu" hidden role="menu">
        ...
        <button type="button" class="is-danger" data-pack-act="delete"><img src="assets/svg/delete.svg" alt=""> Удалить</button>
      </div>`;
    }
  ...
  function cardHTML(pack, own) {
    return `
      <article class="pack-card" data-id="${escapeHtml(pack.id)}" data-own="${own ? "1" : "0"}" data-pinned="${pack.pinned ? "1" : "0"}" data-private="${pack.private ? "1" : "0"}" data-following="${pack.following ? "1" : "0"}" data-updated="${escapeHtml(pack.updatedAt)}" data-title="${escapeHtml(pack.title)}">
  ...
  function openEdit(card) {
    ...
    dialog.innerHTML = `
      <div class="work-dialog-card" role="dialog" aria-modal="true">
        ...
        <form class="work-pack-create" data-pack-edit>
